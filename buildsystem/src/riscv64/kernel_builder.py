...
RV64_LDFLAGS = [
    "-nostdlib", "--eh-frame-hdr", "--relax",
]


class RV64KernelBuilder(Builder):
    ...
    def build(self, dep_objects: Dict[str, List[Path]] | None = None) -> Path:
        ...
        if partic_roots:
            cflags = [
                ...
                "-mcmodel=medium", "-march=rv64gc", "-mabi=lp64d",
        ...
        ld_cmd = [
            tc.linker(), "-T", str(linker_script),
            *RV64_LDFLAGS, "-o", str(output),
        ]
        for obj in all_objs:
...
def _find_compiler_rt(prj_root: Path, arch: str, clang_path: str) -> Path | None:
    rt_name = "libclang_rt.builtins-riscv64.a"
...
BuilderRegistry.register("riscv64", "kernel", RV64KernelBuilder)
