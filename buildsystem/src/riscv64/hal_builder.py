...
def _hal_cflags(debug: bool) -> list[str]:
    return RV64_CFLAGS + (["-O0", "-g"] if debug else ["-O2"])


class RV64HalBuilder(Builder):
    ...
    def build(self) -> List[Path]:
        ...
        for src_rel in self._sources.asm_sources(arch):
            src_abs = self._prj_root / src_rel
            obj = obj_dir / src_rel.with_suffix(".o").name
            cmd = [tc.compiler(), "--target=riscv64-unknown-none", "-ffreestanding",
                   "-nostdlib", "-march=rv64gc", "-mabi=lp64d",
                   "-c", str(src_abs), "-o", str(obj)]
            if self._needs_rebuild(src_abs, obj):
...
BuilderRegistry.register("riscv64", "hal", RV64HalBuilder)
