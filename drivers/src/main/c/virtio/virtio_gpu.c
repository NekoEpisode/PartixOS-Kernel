...
typedef struct virtio_gpu_dev {
    virtq_t          controlq;
    volatile virtio_pci_common_cfg_t *common;   // MMIO VA（locate 已映射）
    volatile void   *notify;
    volatile void   *device;
    uint64_t         cmdbuf_phys;               // 物理（descriptor 用）
    virtio_gpu_ctrl_hdr_t *cmdbuf;              // VA（命令缓冲访问）
...
void virtio_gpu_dev_init(virtio_gpu_dev_t *dev,
    ...
    dev->common = (volatile virtio_pci_common_cfg_t *)(uintptr_t)common_phys;
    dev->notify = (volatile void *)(uintptr_t)notify_phys;
    dev->device = (volatile void *)(uintptr_t)device_phys;
    dev->cmdbuf_phys = cmdbuf_phys;
    dev->cmdbuf = (virtio_gpu_ctrl_hdr_t *)(uintptr_t)phys_to_virt(cmdbuf_phys);
    dev->respbuf_phys = respbuf_phys;
    dev->respbuf = (virtio_gpu_ctrl_hdr_t *)(uintptr_t)phys_to_virt(respbuf_phys);
...
void virtio_gpu_create_2d(virtio_gpu_dev_t *dev, uint32_t id, uint32_t format,
    ...
    virtio_gpu_resource_create_2d_t *cmd =
        (virtio_gpu_resource_create_2d_t *)dev->cmdbuf;
    cmd->resource_id = id;
    cmd->format      = format;
    cmd->width       = width;
    cmd->height      = height;
...
void virtio_gpu_attach_backing(virtio_gpu_dev_t *dev, uint32_t id,
    ...
    virtio_gpu_resource_attach_backing_t *cmd =
        (virtio_gpu_resource_attach_backing_t *)dev->cmdbuf;
    cmd->resource_id = id;
    cmd->nr_entries  = 1;
    cmd->entries[0].addr   = phys;   // 物理地址（GPU DMA 读）
...
void virtio_gpu_set_scanout(virtio_gpu_dev_t *dev, uint32_t scanout,
    ...
    virtio_gpu_set_scanout_t *cmd =
        (virtio_gpu_set_scanout_t *)dev->cmdbuf;
    cmd->r.x          = 0;
    cmd->r.y          = 0;
    cmd->r.width      = width;
    cmd->r.height     = height;
...
void virtio_gpu_transfer_to_host_2d(virtio_gpu_dev_t *dev, uint32_t id,
    ...
    virtio_gpu_transfer_to_host_2d_t *cmd =
        (virtio_gpu_transfer_to_host_2d_t *)dev->cmdbuf;
    cmd->r.x          = x;
    cmd->r.y          = y;
    cmd->r.width      = width;
    cmd->r.height     = height;
...
void virtio_gpu_flush(virtio_gpu_dev_t *dev, uint32_t id,
    ...
    virtio_gpu_resource_flush_t *cmd =
        (virtio_gpu_resource_flush_t *)dev->cmdbuf;
    cmd->r.x          = x;
    cmd->r.y          = y;
    cmd->r.width      = width;
    cmd->r.height     = height;
...
int virtio_gpu_get_display_info(virtio_gpu_dev_t *dev, uint32_t *info_out) {
    ...
    virtio_gpu_resp_display_info_t *resp =
        (virtio_gpu_resp_display_info_t *)dev->respbuf;
...
void virtio_gpu_resource_unref(virtio_gpu_dev_t *dev, uint32_t id) {
    virtio_gpu_resource_unref_t *cmd =
        (virtio_gpu_resource_unref_t *)dev->cmdbuf;
    cmd->resource_id = id;
    cmd->padding = 0;
